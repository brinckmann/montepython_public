"""
.. module:: UltraNest
    :synopsis: Interface the UltraNest nested sampler with Monte Python

This implementation is similar to the MultiNest interface.

UltraNest is a free and open-source nested sampler documented at
...
def run(cosmo, data, command_line):
    ...
    if 'SAMPLER' in os.environ:
        ...
        run_sampler(NS_param_names, loglike, transform=prior, vectorized=False, max_ncalls=1000000)
    else:
        from ultranest import ReactiveNestedSampler
        un_sampler = ReactiveNestedSampler(NS_param_names, loglike, transform=prior, 
            log_dir=base_dir, resume=True,
            #derived_param_names=derived_param_names,
            vectorized=False)
        un_sampler.run(frac_remain=0.5)
        un_sampler.plot()
    #output = nested_run(loglike, prior, **data.NS_arguments)
